...
async function handleRequestPhase(acpClient: AcpClient, job: AcpJob, jobStages: HybridJobStage, role: string) {
    ...
    if (!job.serviceRequirement) {
        console.log(`Job ${job.id} has no service requirement`);
        return;
    }

    console.log(`Responding to job request ${job.id}: ${JSON.stringify(job.serviceRequirement)}`);

    const requirements = job.serviceRequirement || "no requirements";
    ...
    if (!offering) {
        ...
        await job.respond(false, undefined, message as string);
    ...
    if (!targetAgent) {
        ...
        await job.respond(false, undefined, "Target agent not available");
        jobStages.job_phase = "REJECTED";
        return;
    }

    try {
        const offering = targetAgent.offerings[0];
        console.log(`Target offering: ${offering.name}`);
        if (!offering) {
            console.log(`No offerings found for target agent ${agent_name}`);
            await job.respond(false, undefined, "Target agent has no offerings");
            jobStages.job_phase = "REJECTED";
            return;
        }

        const routedJobId = await offering.initiateJob(
            requirements,
            process.env.AGENT_WALLET_ADDRESS as `0x${string}`,
        ...
        await job.respond(false, undefined, "Failed to route job to target agent");
        jobStages.job_phase = "REJECTED";
    }
}
...
async function handleTransactionPhase(acpClient: AcpClient, job: AcpJob, jobStages: HybridJobStage, role: string) {
    ...
    if (memo) {
        console.log(`Paying for job ${mapping.routed_job_id}: ${JSON.stringify(routedJob.serviceRequirement)}`);
        await routedJob.pay(routedJob.price, "Payment for fullfilling the job");
